...
local type_checkers = require "Base.Protobuf.type_checkers"
local encoder = require "Base.Protobuf.encoder"
local decoder = require "Base.Protobuf.decoder"
local listener_mod = require "Base.Protobuf.listener"
local containers = require "Base.Protobuf.containers"
local descriptor = require "Base.Protobuf.descriptor"
local FieldDescriptor = descriptor.FieldDescriptor
local text_format = require "Base.Protobuf.text_format"

--module("protobuf.protobuf")
local _M = {}
local _meta = {__index=_G}
setmetatable(_M, _meta)
local _ENV = _M
    ...
    message_meta._member.MergeFrom = function(self, msg)
        ...
        for field, value in pairs(msg._fields) do
            if field.label == LABEL_REPEATED then
                ...
                for i=1,#value do
                    if (field_value.add) then
                        local t = field_value:add()
                        t:MergeFrom(value[i])
                    else
                        field_value:append(value[i])
                    end
                end
